    ...
    if (user.canteen_id) {
      ...
      if (canteen.is_locked) {
        return res.status(403).json({ 
          ...
          message: canteen.lock_reason || 'Your assigned canteen is currently locked. Please contact an administrator.',
...
exports.getCurrentUser = async (req, res) => {
  try {
    ...
    const user = await User.findById(decoded.userId)
      .select('-password')
      .populate('canteen_id', 'name location type is_locked locked_at lock_reason');
    ...
    if (user.canteen_id && user.canteen_id.is_locked) {
      return res.status(403).json({ 
        error: 'The canteen is locked',
        message: user.canteen_id.lock_reason || 'Your assigned canteen is currently locked.',
