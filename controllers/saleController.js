...
exports.createSale = async (req, res) => {
  try {
    ...
    if (req.user.role === 'manager' && req.user.canteen_id.toString() !== canteen_id) {
      return res.status(403).json({ error: 'You can only create sales for your assigned canteen' });
    }
...
exports.updateSale = async (req, res) => {
  try {
    ...
    if (saleDate.getTime() !== today.getTime()) {
      return res.status(403).json({ error: 'You can only edit sales for today' });
    }
...
exports.getSaleById = async (req, res) => {
  try {
    ...
    if (req.user.role === 'manager' && req.user.canteen_id.toString() !== sale.canteen_id._id.toString()) {
      return res.status(403).json({ error: 'You can only view sales from your canteen' });
    }
...
exports.getSalesByDateAndCanteen = async (req, res) => {
  try {
    ...
    if (req.query.canteen_id) {
      if (req.user.role === 'manager' && req.user.canteen_id.toString() !== req.query.canteen_id) {
        return res.status(403).json({ error: 'You can only view sales from your assigned canteen' });
      }
...
exports.getSalesByDateRange = async (req, res) => {
  try {
    ...
    for (const sale of sales) {
      ...
      const items = await SaleItem.find({ sale_id: sale._id });
      salesByDate[dateStr].total_items += items.reduce((sum, item) => sum + item.quantity, 0);
    }
    ...
    const result = Object.values(salesByDate).map(day => ({
      ...
      online_amount: Math.round((day.online_amount + Number.EPSILON) * 100) / 100,
    }));
